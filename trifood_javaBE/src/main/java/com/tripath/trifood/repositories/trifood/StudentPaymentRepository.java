...
public interface StudentPaymentRepository extends JpaRepository<StudentPayment, Long> {
    ...
    @Query(value = "SELECT COUNT(id)\n" +
            "FROM (meal m INNER JOIN student_orders o ON m.e_group_schedule_id = o.group_schedule_id) \n" +
            "WHERE m.meal_name = o.register_meal\n" +
            "AND m.meal_date = o.order_date\n" +
            "AND o.student_id = ?", nativeQuery = true)
    Integer countStudentDeleteMeal(Integer studentId);

    @Query(value = "SELECT food_name as FoodName, COUNT(food_id)\n" +
            "FROM (meal m INNER JOIN student_orders o ON m.e_group_schedule_id = o.group_schedule_id) \n" +
    ...
    @Query(value = "SELECT student_fullname as StudenName, total_money as TotalMoney, is_paid as IsPaid" +
            "FROM ((student_payments p INNER JOIN students s ON p.student_id = s.id) INNER JOIN eating_classes c ON s.e_class_e_class_id = c.e_class_id) " +
            "WHERE (p.pay_date BETWEEN ? AND ?)", nativeQuery = true)
    List<PaymentManagerService> getAllPayment(String startDate, String endDate);

    @Query(value = "SELECT student_fullname, total_money, is_paid " +
            "FROM ((student_payments p INNER JOIN students s ON p.student_id = s.id) INNER JOIN eating_classes c ON s.e_class_e_class_id = c.e_class_id) " +
